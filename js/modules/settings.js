(function () {
  ...
  async function render() {
    ...
    document.getElementById('save-ebird-key')?.addEventListener('click',async()=>{const key=document.getElementById('ebird-api-key')?.value||'';if(!key.trim()){AppToast.show('请输入新的 Key，或点“恢复内置 Key”');return;}await window.EBirdData.saveApiKey(key);AppToast.show('eBird API Key 已保存（仅保存在本机，设置页不显示完整内容）');await render();});
    document.getElementById('clear-ebird-key')?.addEventListener('click',async()=>{await window.EBirdData.saveApiKey('');const input=document.getElementById('ebird-api-key');if(input)input.value='';AppToast.show('已恢复使用程序内置 eBird API Key');await render();});
    document.getElementById('fetch-ebird')?.addEventListener('click',async()=>{const input=document.getElementById('ebird-api-key');const typed=String(input?.value||'').trim();if(typed)await window.EBirdData.saveApiKey(typed);const key=typed||await window.EBirdData.getApiKey();const button=document.getElementById('fetch-ebird');if(button){button.disabled=true;button.textContent='获取中…';}try{const result=await window.EBirdData.fetchRecent({force:true,apiKey:key});AppToast.show(result.success?`eBird 数据获取完成：${result.hotspotCount} 个观测点、${result.recordCount} 条鸟种记录`:result.error);if(result.success){await AppStateActions.load();await render();}}finally{if(button){button.disabled=false;button.textContent='获取最近7日数据';}}});
